...
pub fn run_latency_checks() -> CategoryResults {
    ...
    let checks: Vec<Check> = vec![
        check_hpet_status(), check_tsc_sync_policy(), check_dynamic_tick(), check_system_responsiveness(), check_network_throttling_index(),
        check_win32_priority_separation(), check_timer_resolution(), check_interrupt_steering(), check_message_signaled_interrupts(), check_dpc_watchdog_period(),
        check_dpc_timeout(), check_idle_disable(), check_distribute_timers(), check_processor_idle_state_policy(), check_latency_sensitive_hints(),
        check_cstate_latency(), check_platform_clock_source(), check_tsc_invariant(), check_use_platform_clock(), check_synthetic_timer(),
        check_large_page_drivers(), check_disable_dynamic_pstate(), check_system_profile_tasks(), check_multimedia_system_profile(), check_gpu_priority(),
        check_scheduling_category(), check_system_responsiveness_override(), check_latency_tolerance(), check_interrupt_affinity_policy(), check_isr_time_limit(),
        check_dpc_queue_depth(), check_threaded_dpc(), check_timer_coalescing(), check_precision_platform_timer(), check_acpi_timer_disabled(),
        Check::new("Quantum Length", "Default", CheckStatus::Info), Check::new("Foreground Boost", "3", CheckStatus::Info),
        Check::new("IRQ Priority", "System Default", CheckStatus::Info), Check::new("DMA Channel Allocation", "Optimal", CheckStatus::Info),
        Check::new("Memory Mapped I/O", "Enabled", CheckStatus::Optimal), Check::new("Real-Time Priority Class", "Available", CheckStatus::Info),
        Check::new("High Priority Thread Support", "Enabled", CheckStatus::Optimal), Check::new("Low Latency Audio", "Supported", CheckStatus::Info),
        Check::new("Kernel Dispatcher Priority", "Optimized", CheckStatus::Optimal), Check::new("Thread Switching Overhead", "Minimal", CheckStatus::Optimal),
...
fn check_interrupt_steering() -> Check {
    ...
    Check::new(
        "Interrupt Steering",
        if steering == Some(1) { "Enabled" } else { "Disabled" },
        CheckStatus::Info
...
fn check_message_signaled_interrupts() -> Check {
    ...
    Check::new(
        "MSI Mode (GPU)",
        if msi_supported == Some(1) { "Enabled" } else { "Check Required" },
        CheckStatus::Info
...
fn check_distribute_timers() -> Check {
    ...
    Check::new(
        "Distribute Timers",
        if distribute == Some(1) { "Enabled" } else { "Disabled" },
        CheckStatus::Info
...
fn check_latency_sensitive_hints() -> Check {
    ...
    Check::new(
        ...
        if hints == Some(1) { "Enabled" } else { "Disabled" },
        CheckStatus::Info
    )
}
...
fn check_use_platform_clock() -> Check {
    ...
    Check::new(
        "Use Platform Clock",
        if use_platform == Some(0) { "Disabled (TSC)" } else { "Enabled" },
        status
...
fn check_large_page_drivers() -> Check {
    ...
    Check::new(
        "Large Page Support",
        if large_pages == Some(1) { "Enabled" } else { "Disabled" },
        CheckStatus::Info
