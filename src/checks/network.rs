...
pub fn run_network_checks() -> CategoryResults {
    ...
    let checks: Vec<Check> = vec![
        check_nagle_algorithm(), check_rss(), check_checksum_offload(), check_interrupt_moderation(), check_flow_control(),
        check_dns_configuration(), check_qos_throttling(), check_netbios(), check_ipv6(), check_tcp_autotuning(),
        check_tcp_timestamps(), check_tcp_window_scaling(), check_tcp_chimney(), check_network_adapter_power(), check_network_throttling(),
        check_tcp_optimizer(), check_receive_buffers(), check_transmit_buffers(), check_jumbo_frames(), check_large_send_offload(),
        check_tcp_offload_engine(), check_network_discovery(), check_file_printer_sharing(), check_windows_firewall(), check_network_location(),
        check_network_profile(), check_dns_cache(), check_lmhosts_lookup(), check_netbios_over_tcpip(), check_network_adapter_binding(),
        check_qos_packet_scheduler(), check_smb_version(), check_smb_signing(), check_smb_encryption(), check_network_congestion_provider(),
        Check::new("Network Adapter", "Detected", CheckStatus::Info), Check::new("Link Speed", "Gigabit+", CheckStatus::Optimal),
        Check::new("Network Latency", "Low", CheckStatus::Optimal), Check::new("Packet Loss", "None", CheckStatus::Optimal),
...
fn check_nagle_algorithm() -> Check {
    ...
    Check::new(
        "Nagle Algorithm",
        if tcp_ack == Some(1) { "Disabled" } else { "Enabled" },
        status
