...
pub fn query_service_status(service_name: &str) -> Option<String> {
    ...
    unsafe {
        ...
        let name: Vec<u16> = OsStr::new(service_name).encode_wide().chain(std::iter::once(0)).collect();
        let service = OpenServiceW(scm, windows::core::PCWSTR(name.as_ptr()), SERVICE_QUERY_STATUS).ok()?;
