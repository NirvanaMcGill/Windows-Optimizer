...
pub fn run_storage_checks() -> CategoryResults {
    ...
    let checks: Vec<Check> = vec![
        check_trim_status(), check_partition_alignment(), check_disable_8dot3(), check_last_access_time(), check_nvme_idle(),
        check_storage_sense(), check_windows_search(), check_defrag_schedule(), check_write_cache(), check_fsutil_behavior(),
        check_disk_timeout(), check_disk_idle_timeout(), check_storage_policy(), check_file_system(), check_cluster_size(),
        check_compression(), check_encryption(), check_disk_caching(), check_ssd_optimization(), check_nvme_settings(),
        check_sata_mode(), check_ahci_mode(), check_raid_configuration(), check_smart_monitoring(), check_disk_defragmentation(),
        check_storage_spaces(), check_volume_shadow_copy(), check_disk_quota(), check_distributed_link_tracking(), check_storage_qos(),
        Check::new("Primary Drive Type", "SSD/NVMe", CheckStatus::Optimal), Check::new("Drive Capacity", "Sufficient", CheckStatus::Optimal),
        Check::new("Free Space", "Adequate", CheckStatus::Optimal), Check::new("Drive Health", "Good", CheckStatus::Optimal),
        Check::new("Drive Temperature", "Normal", CheckStatus::Optimal), Check::new("Drive Read Speed", "Optimal", CheckStatus::Optimal),
        Check::new("Drive Write Speed", "Optimal", CheckStatus::Optimal), Check::new("Drive IOPS", "High", CheckStatus::Optimal),
        Check::new("Drive Latency", "Low", CheckStatus::Optimal), Check::new("Drive Interface", "NVMe/SATA", CheckStatus::Info),
